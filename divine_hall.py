...
async def update_lineup_message():
    if not LINEUP_CHANNEL_ID or not LINEUP_MESSAGE_ID:
        print("Warning: LINEUP_CHANNEL_ID or LINEUP_MESSAGE_ID not set.")
        return
    try:
        channel = await bot.fetch_channel(int(LINEUP_CHANNEL_ID))
        message = await channel.fetch_message(int(LINEUP_MESSAGE_ID))
    except (discord.NotFound, discord.Forbidden, ValueError):
        print("Error: Could not find the channel or message specified in .env.")
        return
...
async def setup_lineup(interaction: discord.Interaction):
    ...
    embed = discord.Embed(title="Leaderboard Initializing...", description="This message will automatically update.", color=0x333333)
...
async def add_category(interaction: discord.Interaction, category_name: str):
    try:
        ...
        conn.rollback()  # THE FIX: Rollback the transaction on any error
        print(f"Error in add_category: {e}")
        await interaction.response.send_message("❌ A database error occurred. The command was cancelled.", ephemeral=True)
